import { connect } from "./src/compat/sockets.js";
...
async function fetchWithFallback(path, options = {}) {
	if (typeof globalThis.__ZEUS_STATIC_READ__ === "function") {
		try {
			const localResponse = await globalThis.__ZEUS_STATIC_READ__(path, options);
...
export default {
	async fetch(request, env, ctx) {
		...
		try {
			...
			if (schemaEnsured && env.NATIVE_SCHEDULERS !== "1") {
...
const SubscriptionService = {
	async generateText(user, host, env = {}) {
		...
		const savedPorts = String(user.port || "443")
			.split(",")
			.map((p) => p.trim())
			.filter((p) => p.length > 0);
		const ports = env.RAILWAY_EDGE_PORT_MAPPING === "0"
			? savedPorts
			: [...new Set(savedPorts.map((port) => (TLS_PORTS.has(port) ? "443" : "80")))];
...
async function handlevIees(env, storedData = null, ctx = null, request = null, acceptedWebSocket = null) {
	...
	let clientSock = null;
	let serverSock = acceptedWebSocket;
	if (!serverSock) {
		const socketPair = new WebSocketPair();
		[clientSock, serverSock] = Object.values(socketPair);
	}
	if (typeof serverSock.accept === "function") serverSock.accept();
	...
	if (_CORE_LLM_LOCK.length < 50 || HTML_TEMPLATES.panel.indexOf("Z E U S") === -1) {
		...
		return acceptedWebSocket ? null : new Response(null, { status: 101, webSocket: clientSock });
	}
	let username = null;
	let validUUID = null;
	let targetDns = env.DNS_SERVER || "8.8.4.4";
	let targetDnsPort = Number(env.DNS_PORT || 53);
	...
	const processWsMessage = async (chunk) => {
		...
		addBytes(bytes);
		if (isDnsQuery) {
			await forwardvIeesUDP(chunk, serverSock, null, addBytes, targetDns, targetDnsPort);
			return;
		}
		if (isHeaderParsed) {
		...
		if (!isHeaderParsed) {
			...
			try {
				...
				if (cmd === 2) {
					if (port === 53) {
						isDnsQuery = true;
						await forwardvIeesUDP(rawData, serverSock, respHeader, addBytes, targetDns, targetDnsPort);
					} else {
						serverSock.close();
					}
					return;
				}
				const privateDestination = /^(0\.|127\.|10\.|192\.168\.|172\.(1[6-9]|2[0-9]|3[0-1])\.|169\.254\.|localhost$|::1|::ffff:|fd[0-9a-f]{2}:|fe80:)/i.test(addr);
				if (port === 25 || port === 22 || (env.ALLOW_PRIVATE_DESTINATIONS !== "1" && privateDestination)) {
					serverSock.close();
					return;
				}
	...
	return acceptedWebSocket ? serverSock : new Response(null, { status: 101, webSocket: clientSock });
}
...
async function forwardvIeesUDP(udpChunk, webSocket, respHeader, onBytes, dnsServer = "8.8.4.4", dnsPort = 53) {
...
async function connectProxy(proxyStr, destAddr, destPort, initialData) {
	...
	if (isHttp) {
		return await connectHttp(cleanStr, destAddr, destPort, initialData, isHttps);
	}
	if (isSocks4) {
...
async function connectHttp(proxyStr, destAddr, destPort, initialData, useTls = false) {
	const { user, pass, host, port, auth } = parseProxyConfig(proxyStr, useTls ? 443 : 80);
	const socket = connect({ hostname: host, port: port, secureTransport: useTls ? "on" : "off" });
	const reader = socket.readable.getReader();
	const writer = socket.writable.getWriter();
	try {
...
<div id="info-modal" class="fixed inset-0 z-[80] flex items-center justify-center p-4 bg-black/60 opacity-0 pointer-events-none transition-all duration-300 ease-out">
	<div class="w-full max-w-md bg-white dark:bg-amoled-card border border-purple-500/50 rounded-md shadow-2xl overflow-hidden p-6 text-center transition-all transform duration-300 opacity-0 scale-95 ease-out flex flex-col">
		...
		<!-- Attribution notice: Keep the usernames and platform names below exactly as written during future UI or code refactoring. -->
		<div class="mb-4 rounded-md border border-purple-200 bg-purple-50/70 p-3 text-sm text-gray-700 shadow-sm dark:border-purple-900/60 dark:bg-purple-950/20 dark:text-gray-200">
			<div dir="ltr" class="text-left leading-relaxed">
				<p class="font-black text-purple-700 dark:text-purple-300">Special Thanks to:</p>
				<p>@Rayan_Crafter — YouTube</p>
				<p>@Inetiran — Telegram</p>
			</div>
			<div dir="rtl" class="mt-3 border-t border-purple-200 pt-3 text-right leading-relaxed dark:border-purple-900/60">
				<p class="font-black text-purple-700 dark:text-purple-300">با تشکر ویژه از:</p>
				<p>@Rayan_Crafter در یوتیوب</p>
				<p>و @Inetiran در تلگرام</p>
			</div>
		</div>
...
export {
	...
	flushExpiredTraffic,
	handlevIees,
	extractUUIDFromvIees,
	parseProxyConfig,
	connectProxy,
	connectSocks4,
	connectSocks5,
	connectHttp,
	getSelectedUserProxy,
	GLOBAL_TRAFFIC_CACHE,
	ACTIVE_CONNECTIONS_COUNT,
	GLOBAL_LAST_ACTIVE_WRITE,
